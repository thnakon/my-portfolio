...
const BentoCard = ({ children, className, isVisible, delay = 0 }) => (
  <div 
    className={`relative group overflow-hidden bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-[32px] p-6 transition-all duration-1000 ${
  ...
  return (
    <div ref={sectionRef} className="mt-32">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 w-full max-w-6xl mx-auto">
        ...
        {/* Card 1: Curated Workbench (Spans 4 cols) */}
        <BentoCard className="md:col-span-4 flex flex-col justify-between min-h-[200px] group/card overflow-hidden" isVisible={isVisible} delay={200}>
          ...
          <div className="relative flex-1 flex items-center justify-center">
             {/* The "Dock" */}
             <div className="flex flex-wrap gap-2 p-3 bg-[var(--bg-primary)]/40 backdrop-blur-md border border-[var(--border-color)] rounded-2xl shadow-xl shadow-black/5 items-center justify-center">
                {siteInfo.tech.map((tech, i) => (
                  <div 
                     key={i} 
                     className="w-10 h-10 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl flex items-center justify-center transition-all duration-500 hover:scale-110 hover:-translate-y-1 hover:shadow-lg group/icon"
                     title={tech.name}
                  >
                     <img 
                       ...
                       alt={tech.name}
                       onError={(e) => {
                         // Fallback for common icon naming mismatches
                         if (tech.icon === 'nextjs') e.target.src = 'https://skillicons.dev/icons?i=next';
                         if (tech.icon === 'tailwindcss') e.target.src = 'https://skillicons.dev/icons?i=tailwind';
                         if (tech.icon === 'framermotion') e.target.src = 'https://skillicons.dev/icons?i=threejs'; 
                       }}
                     />
                  </div>
                ))}
          ...
          <div className="relative mt-4 flex flex-col items-center text-center">
            ...
            <h4 className="text-sm font-heading text-[var(--text-primary)] leading-tight">{t.mySite.toolsTitle}</h4>
          </div>
        </BentoCard>

        {/* Card 2: Guestbook / Note Interaction */}
        <BentoCard className="md:col-span-4 flex flex-col justify-between min-h-[200px] bg-[#080808] border-white/5 group/guest" isVisible={isVisible} delay={400}>
           <div className="relative h-28 mb-2 flex items-center justify-center">
              {/* Cool Ambient Glow */}
              <div className="absolute inset-x-[-20%] inset-y-[-20%] bg-blue-500/5 blur-[50px] rounded-full group-hover/guest:bg-blue-500/10 transition-colors duration-1000" />
              
              {/* Stack of Notes Visual */}
              <div className="relative">
                 {/* Background Note 1 (Tilted Left) */}
                 <div className="absolute top-0 left-1/2 -translate-x-[70%] w-24 h-28 bg-purple-500/5 border border-white/5 rounded-lg -rotate-[15deg] group-hover/guest:-rotate-[20deg] transition-transform duration-700" />
                 
                 {/* Background Note 2 (Tilted Right) */}
                 <div className="absolute top-2 left-1/2 -translate-x-[30%] w-24 h-28 bg-blue-500/5 border border-white/5 rounded-lg rotate-[10deg] group-hover/guest:rotate-[15deg] transition-transform duration-700" />
                 
                 {/* Main Focused Note */}
                 <div className="relative w-28 h-32 bg-[var(--bg-secondary)] border border-white/10 rounded-xl shadow-2xl p-4 flex flex-col group-hover/guest:scale-105 transition-transform duration-500">
                    ...
                    <div className="space-y-2">
                       <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden relative">
                          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-blue-500/30 to-transparent -translate-x-full group-hover/guest:translate-x-full transition-transform duration-1500 ease-in-out" />
                       </div>
                    ...
                    <div className="mt-auto flex justify-between items-center">
                       <div className="flex -space-x-1.5">
                          <div className="w-4 h-4 rounded-full border border-black bg-blue-500/20" />
                          <div className="w-4 h-4 rounded-full border border-black bg-purple-500/20" />
                       </div>
                       <span className="text-[8px] font-mono text-white/10 uppercase tracking-tighter">Interact</span>
                    </div>
                 </div>
              </div>
           </div>
           ...
           <div className="text-center relative z-10">
              ...
              <h4 className="text-sm font-heading text-[var(--text-primary)] tracking-wide">{t.mySite.guestbookTitle}</h4>
           </div>
        </BentoCard>

        {/* Card 3: Vinyl Record Player (Spans 4 cols) */}
        <BentoCard className="md:col-span-4 flex flex-col justify-between min-h-[220px] bg-[#030303] border-white/5 group/music" isVisible={isVisible} delay={600}>
           {/* Visual Backdrop */}
